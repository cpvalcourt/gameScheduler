...
export class TeamInvitationService {
     ...
     */
    static async acceptInvitation(token: string, userId: number): Promise<InvitationResult> {
        try {
            ...
            const isAlreadyMember = await TeamModel.isTeamMember(invitation.team_id, userId);
            if (isAlreadyMember) {
                return {
                    success: false,
            ...
            // Add user to team
            const teamMember = await TeamModel.addMember(invitation.team_id, userId, invitation.invited_role);
     ...
     * @returns Promise<InvitationResult>
     */
    static async declineInvitation(token: string, userId: number): Promise<InvitationResult> {
        try {
            // Find the invitation by token
            const invitation = await TeamInvitationModel.findByToken(token);
            if (!invitation) {
     ...
     * @returns Promise<InvitationResult>
     */
    static async getInvitationByToken(token: string): Promise<InvitationResult> {
        try {
            const invitation = await TeamInvitationModel.findByToken(token);
            if (!invitation) {
                return {
     ...
     * @returns Promise<InvitationResult>
     */
    static async getUserInvitations(userEmail: string): Promise<InvitationResult> {
        try {
            const invitations = await TeamInvitationModel.findByEmail(userEmail);
