...
import { AdvancedSchedulingModel, RecurringPattern, PlayerAvailability } from '../models/advanced-scheduling.model';
...
export class AdvancedSchedulingController {
    ...
    static async createRecurringPattern(req: Request, res: Response) {
        try {
            ...
            const pattern = await AdvancedSchedulingModel.createRecurringPattern({
    ...
    static async generateGamesFromPattern(req: Request, res: Response) {
        try {
            ...
            const pattern = await AdvancedSchedulingModel.getRecurringPattern(parseInt(patternId));
            if (!pattern) {
                return res.status(404).json({ message: 'Recurring pattern not found' });
            }

            // Verify user has access to the series
            const series = await GameSeriesModel.findById(pattern.series_id);
            if (!series || series.created_by !== user.id) {
                return res.status(403).json({ message: 'Not authorized to generate games from this pattern' });
            }

            const generatedGameIds = await AdvancedSchedulingModel.generateGamesFromPattern(
                parseInt(patternId),
                new Date(start_date),
                new Date(end_date)
            );
    ...
    static async getRecurringPatterns(req: Request, res: Response) {
        try {
            const { seriesId } = req.params;
            const user = req.user;
            ...
            const [rows] = await pool.execute(
                'SELECT * FROM RECURRING_PATTERNS WHERE SERIES_ID = ? ORDER BY CREATED_AT DESC',
                [seriesId]
            );
    ...
    static async setPlayerAvailability(req: Request, res: Response) {
        try {
            const {
                date,
                time_slot,
            ...
            const availability = await AdvancedSchedulingModel.setPlayerAvailability({
                user_id: user.id,
                date: new Date(date),
                time_slot,
                status,
    ...
    static async getPlayerAvailability(req: Request, res: Response) {
        try {
            ...
            if (targetUserId !== user.id) {
                ...
                const [teamMembership] = await pool.execute(
                    `SELECT tm.ROLE 
                     ...
                     AND tm1.ROLE IN ('admin', 'captain')`,
                    [user.id, targetUserId, targetUserId]
                );

                if (!(teamMembership as any[]).length) {
                    return res.status(403).json({ message: 'Not authorized to view this user\'s availability' });
                }
            }

            const availability = await AdvancedSchedulingModel.getPlayerAvailability(
                targetUserId,
    ...
    static async detectGameConflicts(req: Request, res: Response) {
        try {
            ...
            const conflicts = await AdvancedSchedulingModel.detectConflicts(parseInt(gameId));
    ...
    static async findOptimalTimeSlot(req: Request, res: Response) {
        try {
            const { seriesId } = req.params;
            ...
            const optimalSlot = await AdvancedSchedulingModel.findOptimalTimeSlot(
                parseInt(seriesId),
                new Date(date),
    ...
    static async getTeamAvailabilitySummary(req: Request, res: Response) {
        try {
            ...
            for (const timeSlot of timeSlots) {
                ...
                for (const row of rows as any[]) {
                    ...
                    else if (row.STATUS === 'unavailable') summary.unavailable = row.count;
                    else if (row.STATUS === 'maybe') summary.maybe = row.count;
                    else summary.notSet = row.count;
                }
