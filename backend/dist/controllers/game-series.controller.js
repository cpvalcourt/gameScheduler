...
exports.GameSeriesController = void 0;
const game_series_model_1 = require("../models/game-series.model");
class GameSeriesController {
    static async createSeries(req, res) {
        try {
            ...
            const series = await game_series_model_1.GameSeriesModel.create({
                name,
                description,
                type,
    ...
    static async getSeries(req, res) {
        try {
            ...
            const series = await game_series_model_1.GameSeriesModel.findById(seriesId);
            if (!series) {
    ...
    static async updateSeries(req, res) {
        try {
            ...
            const isOwner = await game_series_model_1.GameSeriesModel.isOwner(seriesId, user.id);
            if (!isOwner) {
                return res.status(403).json({ message: 'Only the series owner can update the series' });
            }
            const series = await game_series_model_1.GameSeriesModel.update(seriesId, { name, description, type, start_date, end_date });
            if (!series) {
                return res.status(404).json({ message: 'Series not found' });
            }
            res.json({
    ...
    static async getUserSeries(req, res) {
        try {
            const user = req.user;
            if (!user?.id) {
                return res.status(401).json({ message: 'User not authenticated' });
            }
            const series = await game_series_model_1.GameSeriesModel.getUserSeries(user.id);
            res.json(series);
        }
        catch (error) {
...
exports.GameSeriesController = GameSeriesController;
