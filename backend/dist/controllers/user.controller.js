...
class UserController {
    static async getCurrentUser(req, res) {
        ...
        const dbUser = await user_model_1.UserModel.findById(user.id);
        if (!dbUser) {
    ...
    static async updateProfile(req, res) {
        try {
            ...
            const { username, email, bio, location, phone, date_of_birth, linkedin_url, twitter_url, website_url } = req.body;
            ...
            if (twitter_url !== undefined)
                updateData.twitter_url = twitter_url;
            ...
            const updatedUser = await user_model_1.UserModel.update(user.id, updateData);
            if (!updatedUser) {
                return res.status(404).json({ message: 'User not found' });
            }
            return res.json({
    ...
    static async changePassword(req, res) {
        try {
            ...
            await database_1.pool.execute('UPDATE USERS SET PASSWORD_HASH = ?, UPDATED_AT = NOW() WHERE ID = ?', [hashedNewPassword, user.id]);
            return res.json({
    ...
    static async updateUser(req, res) {
        ...
        const updated = await user_model_1.UserModel.update(id, updates);
        if (!updated) {
            return res.status(404).json({ message: 'User not found' });
        }
    ...
    static async uploadProfilePicture(req, res) {
        try {
            ...
            const imagePath = await (0, upload_1.processAndSaveImage)(req.file.buffer, req.file.originalname, 300, // width
            300 // height
            );
            // Delete old profile picture if it exists
            if (currentUser.profile_picture_url) {
                await (0, upload_1.deleteImageFile)(currentUser.profile_picture_url);
            }
            // Update user's profile picture URL
            const updatedUser = await user_model_1.UserModel.update(user.id, {
    ...
    static async deleteAccount(req, res) {
        try {
            ...
            const isPasswordValid = await bcryptjs_1.default.compare(password, currentUser.password);
            if (!isPasswordValid) {
            ...
            const deleted = await user_model_1.UserModel.delete(user.id);
            if (!deleted) {
                return res.status(404).json({ message: 'User not found' });
            }
            return res.json({
